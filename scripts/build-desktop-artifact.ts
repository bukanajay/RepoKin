...
const DESKTOP_APP_ID = "com.repokin.app";
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "RepoKin (Nightly)"
    : (desktopPackageJson.productName ?? "RepoKin (Alpha)");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "RepoKin-${version}-${arch}.${ext}",
  ...
  if (platform === "mac") {
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "RepoKin",
          schemes: ["t3code", "t3code-dev", "repokin", "repokin-dev"],
        },
      ],
      ...(macPasskeySigning
        ? {
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "repokin",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // t3code:// / repokin:// OAuth callbacks to the app.
      protocols: [
        {
          name: "RepoKin",
          schemes: ["t3code", "t3code-dev", "repokin", "repokin-dev"],
